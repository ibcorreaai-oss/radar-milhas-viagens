...
export async function saveStayToBucketList(stayId: string): Promise<void> {
  ...
  const { error } = await supabase.from('bucket_list_items').insert({ bucket_list_id: bucketListId, stay_id: stayId });

  // 23505 = unique_violation — já estava salvo, não é erro do ponto de
  // vista do usuário (índice único bucket_list_items_unique_stay evita duplicata).
